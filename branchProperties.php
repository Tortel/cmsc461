...
if(!$branch || !is_numeric($branch)){
   ...
   while( ($associateRow = dbFetchRow($associates)) ){
      ...
      $propertyQuery = dbExec($db, "Select Property.street, Property.city, property.state, Property.zip, type, bedrooms, bathrooms, sqfoot, rent, name, Owner.id from Property, owner where property.owner = owner.id and rented = 'N' and property.associate = $row[0] order by rent asc");
